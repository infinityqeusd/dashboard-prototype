...
export function TickerRibbon({ items }: TickerRibbonProps) {
  ...
  return (
    <section className="px-4 pt-4 sm:px-6 lg:px-8">
      <div className="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white/80 py-2 shadow-panel backdrop-blur-xl dark:border-slate-700/70 dark:bg-slate-950/60 dark:shadow-panel-dark">
        ...
        <div className="ticker-track flex w-max items-center gap-2 px-2 hover:[animation-play-state:paused]">
          {loopItems.map((item, index) => (
            <div
              key={`${item.ticker}-${index}`}
              className="flex shrink-0 items-center gap-2 rounded-xl border border-slate-200/70 bg-white/90 px-3 py-2 text-sm dark:border-slate-700/80 dark:bg-slate-900/90"
            >
